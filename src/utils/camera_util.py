...
def center_looking_at_camera_pose(camera_position: torch.Tensor, look_at: torch.Tensor = None,
...
def get_zero123plus_input_cameras(batch_size=1, radius=4.0, fov=30.0):
    ...
    azimuths = np.array([17.14285714, 34.28571429, 51.42857143, 68.57142857, 85.71428571,
                         102.85714286, 120., 137.14285714, 154.28571429, 171.42857143,
                         188.57142857, 205.71428571, 222.85714286, 240., 257.14285714,
                         274.28571429, 291.42857143, 308.57142857, 325.71428571, 342.85714286,
                         0.]).astype(float)

    elevations = np.array([10, 10, 10, 10, 10, 10,
                           10, 10, 10, 10, 10, 10,
                           10, 10, 10, 10, 10, 10,
    ...
    Ks = FOV_to_intrinsics(fov).unsqueeze(0).repeat(21, 1, 1).float().flatten(-2)

    extrinsics = c2ws[:, :12]
    intrinsics = torch.stack([Ks[:, 0], Ks[:, 4], Ks[:, 2], Ks[:, 5]], dim=-1)
    print('intrinsics', intrinsics)
    print('c2ws', extrinsics)
