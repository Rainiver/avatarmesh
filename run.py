...
for idx, sample in enumerate(outputs):
    ...
    filenames = sorted([f for f in os.listdir(image_folder_path) if f.endswith('.png')])
    images_list = []
    for f in filenames:
        img = Image.open(os.path.join(image_folder_path, f)).convert('RGB')
        img_tensor = transforms.Compose([
            transforms.ToTensor(),  # 会将 [0, 255] 转换为 [0.0, 1.0]
        ])(img)
        images_list.append(img_tensor)
    images = torch.stack(images_list).to(device).unsqueeze(0)
    ...
    print('input_cameras.shape', input_cameras.shape)
